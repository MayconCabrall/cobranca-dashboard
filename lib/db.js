...
export async function listarEnvios({ status, regua_id, page = 1, limit = 50 } = {}) {
  let query = sb()
    .from('envios')
    .select('id, status, erro, enviado_em, faturas(valor, vencimento, clientes(nome, telefone)), reguas(nome)', { count: 'exact' })
...
export async function triggerSync() {
  const { data, error } = await sb().functions.invoke('sync-clientes')
  if (error) throw new Error(error.message)
  return data
}
